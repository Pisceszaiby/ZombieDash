...
def guess_password():
    ...
    label = Label(game, bg='olivedrab4', fg='white', relief='flat',text=f"Guess The Complete Password |||{rand1}?||| \n Before The Zombies Eat You Alive")
    label.pack()
    entry = Entry(game,bg='olivedrab4',fg='white',width=20, borderwidth=4)    #user will guess the password here
    entry.pack()
    ...
    def destroy_win(result):
        ...
        if result=='won':       #if user correctly guessed the password within 3 tries
            result_label=Label(final_result,fg='white',bg='olivedrab3', font=("Chiller",'25','bold') , text=f'CONGRATULATIONS {name}!! You Escaped' )
            result_label.place(x=100,y=500)
        else:                   #if user was unable to guess the password
            result_label=Label(final_result,fg='white',bg='olivedrab3', font=("Chiller",'25','bold') , text=f'Unfortunately {name} You Didn\'t Survive The Apocalypse')
    ...
    button_check = Button(game,bg='olivedrab4',fg='white', text="Check", command=check_rand)
    button_check.pack()
    message = StringVar()    
